import numpy as np
import random
import csv
import pandas as pd
from tabulate import tabulate

class AlgorithmParameters:
    def __init__(self, positive_indices, negative_indices, diversification_index, investment_time_index):
        self.positive_indices = positive_indices
        self.negative_indices = negative_indices
        self.diversification_index = diversification_index
        self.investment_time_index = investment_time_index

class CriteriaManager:
    def __init__(self, data_file, criteria_file, file_path):
        self.data_file = data_file
        self.criteria_file = criteria_file
        self.file_path = file_path
        self.criteria_data = []
        self.normalized_scores = []
        self.weights = []
        self.weights_percent = []
        self.criteria_values = []#
        self.investment_names = []
        self.criteria_estimated = []
        self.risk_estimated = []

    def read_csv_to_array(self):
        values = []
        try:
            with open(self.data_file, mode='r') as file:
                reader = csv.reader(file)
                next(reader)  # Bỏ qua dòng tiêu đề
                for row in reader:
                    values.append(float(row[0]))
        except FileNotFoundError:
            print(f"File not found: {self.data_file}. Please check the file path.")
        return values

    def normalize_to_nine_scale(self, data):
        max_value = max(data)
        min_value = min(data)
        normalized = []

        for value in data:
            if max_value == min_value:
                normalized.append(5)
            else:
                normalized_value = 1 + 8 * (value - min_value) / (max_value - min_value)
                normalized.append(round(normalized_value))

        return normalized

    def create_comparison_matrix(self, normalized_scores):
        n = len(normalized_scores)
        matrix = np.ones((n, n))

        for i in range(n):
            for j in range(n):
                if i != j:
                    matrix[i][j] = normalized_scores[i] / normalized_scores[j]

        return matrix

    def compute_weights(self):
        # Tính toán trọng số từ ma trận so sánh
        self.criteria_data = self.read_csv_to_array()
        self.normalized_scores = self.normalize_to_nine_scale(self.criteria_data)
        comparison_matrix = self.create_comparison_matrix(self.normalized_scores)

        # Tính toán ma trận chuẩn hóa
        col_sum = np.sum(comparison_matrix, axis=0)
        normalized_matrix = comparison_matrix / col_sum

        # Tính trọng số trung bình
        self.weights = np.mean(normalized_matrix, axis=1)
        self.weights_percent = self.weights / np.sum(self.weights) * 100
        return self.weights

    def load_criteria_values(self):
        file_path1 = "criteria_values.csv"
        try:
            data = pd.read_csv(file_path1, encoding="utf-8")

            # Chuyển dữ liệu thành mảng NumPy (hoặc giữ lại dưới dạng DataFrame nếu cần)
            self.criteria_values = data.to_numpy()  # Chuyển sang mảng NumPy

        except FileNotFoundError:
            print(f"Không tìm thấy file: {file_path1}. Vui lòng kiểm tra lại.")

    def load_investment_data(self):
        try:
            file_path = "criteria_and_risk_investment.csv"
            df = pd.read_csv(file_path, encoding="utf-8")

            self.investment_names = df["Danh mục"]
            self.criteria_estimated = df["Criteria Estimated"]
            self.risk_estimated = df["Risk Estimated"]
        except FileNotFoundError:
            print(f"File not found: {self.file_path}. Please check the file path.")

class IndividualGenerator:
    def __init__(self, criteria_manager, pop_size):
        self.criteria_manager = criteria_manager
        self.pop_size = pop_size

    # Xác định số biến n_bien dựa trên weights
    def determine_n_bien(self):
        sorted_indices = np.argsort(self.criteria_manager.weights)[-3:]
        return 10 if 6 in sorted_indices else 6

    # Hàm tạo 1 cá thể
    def create_individual(self, n_bien):
        n = n_bien // 2  # Số lượng (xi, yi)
        individual = [0] * n_bien  # Cá thể chứa num_vars giá trị
        # Tạo các giá trị xi duy nhất ở vị trí chẵn
        xi_values = random.sample(range(100), n)  # Lấy n giá trị không trùng từ [0, 99]
        for i, xi in enumerate(xi_values):
            individual[2 * i] = xi
        # Tạo các giá trị yi bằng phân phối Dirichlet
        yi_values = np.random.dirichlet(np.ones(n)) * 100  # Tổng các giá trị luôn là 100
        for i, yi in enumerate(yi_values):
            individual[2 * i + 1] = np.round(yi, 2)
        return individual
    def generate_population(self):
        n_bien = self.determine_n_bien()
        return [self.create_individual(n_bien) for _ in range(pop_size)]

class Fitness_Function_Investment:
    def __init__(self, criteria_manager, parameters):
        self.criteria_manager = criteria_manager
        self.parameters = parameters

    def fitness_function(self, i):
        if i < 0 or i >= self.criteria_manager.criteria_values.shape[0]:
            raise IndexError("Index i is out of bounds for criteria_values.")

        current_criteria_values = self.criteria_manager.criteria_values[i]
        sorted_indices = np.argsort(self.criteria_manager.weights)[-3:]

        positive_contribution = np.dot(
            [self.criteria_manager.weights[idx] for idx in self.parameters.positive_indices],
            [current_criteria_values[idx] for idx in self.parameters.positive_indices]
        ) * np.sum(self.criteria_manager.weights_percent[self.parameters.positive_indices])

        negative_contribution = np.dot(
            [self.criteria_manager.weights[idx] for idx in self.parameters.negative_indices],
            [current_criteria_values[idx] for idx in self.parameters.negative_indices]
        ) * np.sum(self.criteria_manager.weights_percent[self.parameters.negative_indices])

        diversification_contribution = self.criteria_manager.weights[self.parameters.diversification_index] * current_criteria_values[self.parameters.diversification_index] * self.criteria_manager.weights_percent[self.parameters.diversification_index] \
            if self.parameters.diversification_index in sorted_indices else -0.5*self.criteria_manager.weights[self.parameters.diversification_index] * current_criteria_values[self.parameters.diversification_index] * self.criteria_manager.weights_percent[self.parameters.diversification_index]

        investment_time_contribution = self.criteria_manager.weights[self.parameters.investment_time_index] * current_criteria_values[self.parameters.investment_time_index] * self.criteria_manager.weights_percent[self.parameters.investment_time_index] \
            if self.parameters.investment_time_index in sorted_indices else -0.5*self.criteria_manager.weights[self.parameters.investment_time_index] * current_criteria_values[self.parameters.investment_time_index] * self.criteria_manager.weights_percent[self.parameters.investment_time_index]

        return positive_contribution - negative_contribution + diversification_contribution + investment_time_contribution

class Caculate_Fitness_Individual_Generator:
    @staticmethod
    def calculate_fitness(fitness_investment, population):
        fitness_values = []  # Danh sách chứa fitness của từng cá thể

        for individual in population:
            individual_fitness = 0
            for j in range(0, len(individual), 2):  # Duyệt qua các giá trị xi và yi (xi: chẵn, yi: lẻ)
                xi_value = individual[j]  # Lấy xi (vị trí chẵn)
                yi_value = individual[j + 1]  # Lấy yi (vị trí lẻ)

                # Đảm bảo xi_value là số nguyên và nằm trong phạm vi hợp lệ
                if isinstance(xi_value, int) and 0 <= xi_value < len(fitness_investment):
                    fitness_xi = fitness_investment[xi_value]  # Giá trị fitness từ danh sách/ma trận
                else:
                    fitness_xi = 0  # Gán giá trị mặc định nếu xi không hợp lệ

                # Đảm bảo yi_value không phải None
                if yi_value is not None:
                    individual_fitness += fitness_xi * (yi_value / 100)  # Tính đóng góp của xi và yi

            fitness_values.append(individual_fitness)  # Lưu tổng fitness của cá thể này vào danh sách

        return fitness_values

class MVO:
    def __init__(self, bounds, WEP, TDR):
        self.bounds = bounds  # Giới hạn của không gian tìm kiếm
        self.WEP = WEP        # Tỷ lệ White Hole / Wormhole
        self.TDR = TDR        # Tỷ lệ chuyển động

    # Chuẩn hóa tỷ lệ NI cho MVO
    def normalize_inflation_rate(self, fitness):
        if not isinstance(fitness, np.ndarray):
            fitness = np.array(fitness)
        total_fitness = np.sum(fitness)
        if total_fitness == 0:
            return np.ones_like(fitness) / len(fitness)

        epsilon = 1e-6
        normalized_fitness = 1 / (fitness + epsilon)
        normalized_fitness /= np.sum(normalized_fitness)
        return normalized_fitness

    # Lựa chọn bằng cách quay bánh xe
    def roulette_wheel_selection(self, m):
        cumulative_sum = np.cumsum(m - np.min(m))  # Tránh giá trị âm
        normalized_weights = cumulative_sum / cumulative_sum[-1]  # Chuẩn hóa
        random_number = np.random.rand()
        index = np.searchsorted(normalized_weights, random_number)
        return index

    # Cập nhật dân số bằng cơ chế của MVO
    def update_population(self, population, fitness, best_solution):
        new_population = population.copy()
        best_fitness = np.min(fitness)

        # Tính NI
        NI = self.normalize_inflation_rate(fitness)
        sorted_indices = np.argsort(fitness)
        SU = [population[i] for i in sorted_indices]  # Sắp xếp theo fitness

        for i in range(len(population)):
            if fitness[i] > best_fitness:  # Bỏ qua cá thể tốt nhất
                r1 = np.random.rand()
                if r1 < NI[i]:  # Cơ chế White Hole
                    white_hole_index = self.roulette_wheel_selection(-NI)
                    new_population[i] = SU[white_hole_index]
                else:  # Cơ chế Wormhole
                    n = len(best_solution) // 2  # Số lượng (xi, yi)
                    new_individual = population[i][:]  # Khởi tạo cá thể mới từ cá thể gốc (giữ nguyên xi)
                    new_yis = []
                    r2 = np.random.uniform(0, 1)
                    if r2 < self.WEP:
                        for j in range(n):
                            r3 = np.random.uniform(0, 1)
                            r4 = np.random.uniform(0, 1)
                            if r3 < 0.5:
                                yi_value = np.round(
                                    best_solution[2 * j + 1] + self.TDR * ((self.bounds[1] - self.bounds[0]) * r4 + self.bounds[0]), 2)
                                new_yis.append(yi_value)
                            else:
                                yi_value = np.round(
                                    best_solution[2 * j + 1] - self.TDR * ((self.bounds[1] - self.bounds[0]) * r4 + self.bounds[0]), 2)
                                new_yis.append(yi_value)

                        # Đảm bảo tổng các giá trị yi = 100%
                        total_yi = np.sum(new_yis)
                        if total_yi != 100:
                            diff = 100 - total_yi
                            new_yis = [yi + (diff / len(new_yis)) for yi in new_yis]

                        # Kiểm tra lại kích thước của new_individual trước khi cập nhật
                        for j in range(n):
                            if 2 * j + 1 < len(new_individual):
                                new_individual[2 * j + 1] = new_yis[j]

                        new_population[i] = new_individual
            else:
                new_population[i] = population[i]  # Giữ nguyên cá thể gốc nếu là tốt nhất

        return new_population

class COA:
    def __init__(self, n_bien, fitness_investment, pop_size):
        self.n_bien = n_bien  # Số lượng biến
        self.fitness_investment = fitness_investment  # Mảng giá trị fitness
        self.pop_size = pop_size  # Kích thước quần thể

    def create_individual(self):
        # Hàm tạo cá thể mới (Cần định nghĩa chi tiết)
        return [random.randint(0, 99) for _ in range(self.n_bien)]

    def update_population(self, population, fitness, best_solution):
        r = random.random()
        I = random.choice([1, 2])

        new_population_coa = population.copy()

        # Iguana là cá thể tốt nhất (best solution)
        iguana = best_solution

        for i in range(self.pop_size):
            if i >= self.pop_size / 2:  # Iguana fall down
                iguana_G = self.create_individual()

                individual_fitness_G = 0
                for j in range(0, len(iguana_G), 2):  # Duyệt qua các giá trị xi và yi (xi: chẵn, yi: lẻ)
                    xi_value_iguana = iguana_G[j]  # Lấy xi (vị trí chẵn)
                    yi_value_iguana = iguana_G[j + 1]  # Lấy yi (vị trí lẻ)

                    # Đảm bảo xi_value là số nguyên và nằm trong phạm vi hợp lệ
                    if isinstance(xi_value_iguana, int) and 0 <= xi_value_iguana < len(self.fitness_investment):
                        fitness_xi = self.fitness_investment[xi_value_iguana]  # Giá trị fitness từ danh sách/ma trận
                        individual_fitness_G += fitness_xi * yi_value_iguana  # Tính đóng góp của xi và yi

                for j in range(0, len(population[i]), 2):  # Duyệt qua từng xi, yi
                    n = self.n_bien // 2
                    new_individual = [None] * (2 * n)
                    xi_value = population[i][j]  # Lấy xi
                    new_individual[j + 1] = population[i][j + 1]  # Lấy yi

                    retries = 0
                    found = False
                    max_retries = 100

                    used_xi = set()  # Tập hợp để theo dõi các giá trị xi đã được tạo
                    new_xis = []  # Danh sách tạm thời để lưu các giá trị new_xi

                    # Tạo danh sách các giá trị khả thi trước
                    possible_xis = list(range(0, 100))  # Giới hạn phạm vi từ 0 đến 99

                    for h in range(0, len(iguana_G), 2):
                        # Cập nhật xi và yi dựa trên điều kiện fitness
                        if individual_fitness_G < fitness[i]:
                            while retries < max_retries:
                                # Cập nhật xi và yi khi fitness của iguana_G nhỏ hơn fitness cá thể hiện tại
                                new_xi = xi_value + r * (iguana_G[h] - I * np.ones_like(xi_value))
                                new_xi = int(round(new_xi))

                                if new_xi < 0:
                                    new_xi = 0
                                elif new_xi > 99:
                                    new_xi = 99

                                # Nếu không trùng, thêm vào danh sách và thoát khỏi vòng lặp
                                if new_xi not in used_xi and new_xi in possible_xis:
                                    used_xi.add(new_xi)
                                    new_xis.append(new_xi)
                                    found = True
                                    break
                                retries += 1

                            if not found:  # Nếu không tìm thấy sau max_retries lần thử, gán giá trị mặc định
                                new_xi = random.choice([xi for xi in possible_xis if xi not in used_xi])
                                used_xi.add(new_xi)
                                new_xis.append(new_xi)

                            # Gán giá trị mới vào cá thể
                            for k, new_xi in enumerate(new_xis):
                                new_individual[2 * k] = new_xi

                            # Thêm cá thể mới vào quần thể
                            new_population_coa.append(new_individual)

                        else:
                            while retries < max_retries:
                                # Cập nhật xi và yi khi fitness của iguana_G lớn hơn hoặc bằng fitness cá thể hiện tại
                                new_xi = xi_value + r * (np.ones_like(xi_value) - iguana_G[h])
                                new_xi = int(round(new_xi))

                                if new_xi < 0:
                                    new_xi = 0
                                elif new_xi > 99:
                                    new_xi = 99

                                # Nếu không trùng, thêm vào danh sách và thoát khỏi vòng lặp
                                if new_xi not in used_xi and new_xi in possible_xis:
                                    used_xi.add(new_xi)
                                    new_xis.append(new_xi)
                                    found = True
                                    break
                                retries += 1

                            if not found:  # Nếu không tìm thấy sau max_retries lần thử, gán giá trị mặc định
                                new_xi = random.choice([xi for xi in possible_xis if xi not in used_xi])
                                used_xi.add(new_xi)
                                new_xis.append(new_xi)

                            # Gán giá trị mới vào cá thể
                            for k, new_xi in enumerate(new_xis):
                                new_individual[2 * k] = new_xi

                            # Thêm cá thể mới vào quần thể
                            new_population_coa.append(new_individual)

            else:
                # Coati trên cây đe dọa Iguana
                for j in range(0, len(population[i]), 2):
                    n = self.n_bien // 2
                    new_individual = [None] * (2 * n)
                    xi_value = population[i][j]  # Lấy xi
                    new_individual[j + 1] = population[i][j + 1]  # Lấy yi

                    retries = 0
                    found = False
                    max_retries = 100

                    used_xi = set()  # Tập hợp để theo dõi các giá trị xi đã được tạo
                    new_xis = []  # Danh sách tạm thời để lưu các giá trị new_xi

                    # Tạo danh sách các giá trị khả thi trước
                    possible_xis = list(range(0, 100))  # Giới hạn phạm vi từ 0 đến 19

                    for h in range(0, len(iguana), 2):
                        while retries < max_retries:
                            new_xi = xi_value + r * (iguana[h] - I * np.ones_like(xi_value))
                            new_xi = int(round(new_xi))

                            if new_xi < 0:
                                new_xi = 0
                            elif new_xi > 99:
                                new_xi = 99

                            # Nếu không trùng, thêm vào danh sách và thoát khỏi vòng lặp
                            if new_xi not in used_xi and new_xi in possible_xis:
                                used_xi.add(new_xi)
                                new_xis.append(new_xi)
                                found = True
                                break
                            retries += 1

                        if not found:  # Nếu không tìm thấy sau max_retries lần thử, gán giá trị mặc định
                            new_xi = random.choice([xi for xi in possible_xis if xi not in used_xi])
                            used_xi.add(new_xi)
                            new_xis.append(new_xi)

                        # Gán giá trị mới vào cá thể
                        for k, new_xi in enumerate(new_xis):
                            new_individual[2 * k] = new_xi

                        # Thêm cá thể mới vào quần thể
                        new_population_coa.append(new_individual)

        return new_population_coa

class HybridCOA_MVO:
    def __init__(self, population, fitness, bounds, max_iter, switch_iter, fitness_investment):
        self.population = population  # Quần thể
        self.fitness = fitness  # Giá trị fitness ban đầu
        self.bounds = bounds  # Giới hạn
        self.max_iter = max_iter  # Số vòng lặp tối đa
        self.switch_iter = switch_iter  # Số vòng lặp chuyển đổi từ MVO sang COA
        self.fitness_investment = fitness_investment  # Mảng giá trị fitness cho các cá thể
        self.best_solution = self.population[np.argmin(self.fitness)].copy()  # Cá thể tốt nhất
        self.best_fitness = np.min(self.fitness)  # Giá trị fitness tốt nhất
        self.WEPmin = 0.2  # WEP tối thiểu
        self.WEPmax = 0.8  # WEP tối đa
        self.TDR = 1.0  # TDR ban đầu

    def update_population_mvo(self, WEP, TDR):
        # Hàm cập nhật dân số theo chiến lược MVO
        # (Cần xây dựng chi tiết cách cập nhật dân số MVO)
        new_population = self.population  # Placeholder
        return new_population

    def update_population_coa(self):
        # Hàm cập nhật dân số theo chiến lược COA
        # (Cần xây dựng chi tiết cách cập nhật dân số COA)
        new_population = self.population  # Placeholder
        return new_population

    def run(self):
        for iter in range(self.max_iter):
            # Cập nhật WEP và TDR
            WEP = self.WEPmin + (iter / self.max_iter) * (self.WEPmax - self.WEPmin)
            self.TDR = 1 - (iter / self.max_iter)

            # Giai đoạn cập nhật dân số
            if iter < self.switch_iter:
                # Giai đoạn MVO
                new_population = self.update_population_mvo(WEP, self.TDR)
            else:
                # Giai đoạn COA
                new_population = self.update_population_coa()

            # Tính fitness của dân số mới
            fitness_current = Caculate_Fitness_Individual_Generator.calculate_fitness(fitness_investment, new_population)
            current_best_idx = np.argmin(fitness_current)
            current_best_fitness = fitness_current[current_best_idx]

            # Cập nhật giải pháp tốt nhất
            if current_best_fitness < self.best_fitness:
                self.best_fitness = current_best_fitness
                self.best_solution = self.population[current_best_idx].copy()

        return self.best_fitness, self.best_solution

def display_ahp_results(comparison_matrix, weights1):
    # Kiểm tra ma trận so sánh và trọng số
    if comparison_matrix is None:
        print("Error: Comparison matrix is None!")
        return
    if weights is None:
        print("Error: Weights are None!")
        return

    # Hiển thị ma trận so sánh
    headers = ["Criterias", "Expected Return", "Risk", "Liquidity", "Investment Time", "Cost",
               "Stability of Cash Flow", "Diversification"]
    index = ["Expected Return", "Risk", "Liquidity", "Investment Time", "Cost", "Stability of Cash Flow",
             "Diversification"]

    # Kiểm tra kích thước của ma trận so sánh
    if comparison_matrix.shape[0] != len(index):
        print("Warning: Number of rows in comparison matrix does not match number of criteria.")

    data = []
    for i in range(comparison_matrix.shape[0]):
        temp_val = list(comparison_matrix[i, :])
        temp_val.insert(0, index[i])
        data.append(temp_val)

    print("Kết quả Ma Trận AHP")
    print(tabulate(data, headers=headers, tablefmt='fancy_grid', numalign="center"))

    print("\n")
    print("Trọng số cuối cùng cho các tiêu chí")

    index2 = ["Expected Return", "Risk", "Liquidity", "Investment Horizon", "Cost",
              "Stability of Cash Flow", "Diversification"]

    data2 = []
    rank = [sorted(weights, reverse=True).index(x) + 1 for x in weights]

    for i in range(len(weights)):
        data2.append([index2[i], weights[i], rank[i]])

    # Sắp xếp dữ liệu từ lớn đến nhỏ theo giá trị weights
    data2_sorted = sorted(data2, key=lambda x: x[1], reverse=True)

    print(tabulate(data2_sorted, headers=["Criterias", "Criteria Weights", "Rank"], tablefmt='fancy_grid',
                   numalign="center"))

class DisplayInvestmentsByCategory:
    def __init__(self, replacement_map, categories, criteria_manager):
        self.replacement_map = self.read_replacement_map_from_csv(replacement_map)
        self.categories = self.read_categories_from_csv(categories)
        self.criteria_manager = criteria_manager

        # Sử dụng dữ liệu từ CriteriaManager
        self.investment_names = self.criteria_manager.investment_names
        self.criteria_estimated = self.criteria_manager.criteria_estimated
        self.risk_estimated = self.criteria_manager.risk_estimated

    def read_replacement_map_from_csv(self, replacement_map_file):
        replacement_map = {}
        df = pd.read_csv(replacement_map_file)

        # Duyệt qua từng dòng trong DataFrame để tạo dictionary
        for index, row in df.iterrows():
            category = row['Danh mục']
            subcategories = row['Các mục thay thế'].split(", ")  # Tách danh sách các mục thay thế từ chuỗi
            replacement_map[category] = subcategories

        return replacement_map

    def read_categories_from_csv(self, categories_file):
        categories = {}
        with open(categories_file, mode='r', encoding='utf-8') as file:
            reader = csv.reader(file)
            next(reader)  # Bỏ qua dòng tiêu đề
            for row in reader:
                category, subcategory = row
                if category not in categories:
                    categories[category] = []
                categories[category].append(subcategory)
        return categories

    def classify_investments(self, best_solution_investment):
        from collections import defaultdict
        import random

        classified_investments = defaultdict(list)

        # Chuyển danh sách categories thành chữ thường để so khớp dễ dàng
        categories_lower = {
            category: [name.lower() for name in names] for category, names in self.categories.items()
        }

        default_category = "Khác"  # Dự phòng nếu không phân loại được

        for i in range(0, len(best_solution_investment), 2):
            xi_investment = best_solution_investment[i]
            yi_investment = best_solution_investment[i + 1]

            # Kiểm tra phạm vi hợp lệ của xi_investment
            if not (0 <= xi_investment < len(self.investment_names)):
                xi_investment = random.randint(0, len(self.investment_names) - 1)

            # Chuyển thành chữ thường và viết hoa chữ cái đầu tiên
            investment_name = self.investment_names[xi_investment].lower().capitalize()

            # Phân loại dựa trên ánh xạ nhanh
            matched = False
            for category, names in categories_lower.items():
                if investment_name in names:
                    classified_investments[category].append((investment_name, yi_investment))
                    matched = True
                    break

            # Nếu không phân loại được, đưa vào nhóm "Khác"
            if not matched:
                classified_investments[default_category].append((investment_name, yi_investment))

        # Sắp xếp danh mục theo giá trị yi_investment (giảm dần)
        for category in classified_investments:
            classified_investments[category].sort(key=lambda x: x[1], reverse=True)

        return classified_investments

    def display_investments_by_category(self, classified_investments):
        from tabulate import tabulate
        import random

        # Đảm bảo không có danh mục giống nhau trong mỗi danh mục đầu tư
        used_investments = set()  # Lưu trữ các danh mục đã sử dụng
        selected_investments = []  # Danh sách lưu các đầu tư đã chọn

        # Lặp qua từng danh mục trong classified_investments
        for category, investments in classified_investments.items():
            # Tạo bản sao của investments để tránh thay đổi dữ liệu gốc
            investments_copy = investments[:]

            # Lặp qua từng khoản đầu tư để xử lý thay thế nếu cần thiết
            for i in range(len(investments_copy)):
                investment_name, yi_investment = investments_copy[i]

                # Kiểm tra xem tiêu chí đa dạng hóa có nằm trong top 3 không
                if 6 in sorted_indices and investment_name in self.replacement_map:
                    # Chỉ thực hiện thay thế khi điều kiện đúng
                    available_replacements = [
                        name for name in self.replacement_map[investment_name]
                        if name not in used_investments
                    ]
                    if available_replacements:
                        new_name = random.choice(available_replacements)
                        if (new_name, yi_investment, category) not in selected_investments:
                            selected_investments.append((new_name, yi_investment, category))
                        used_investments.add(new_name)
                    else:
                        # Nếu không thuộc điều kiện, thêm danh mục gốc
                        if (investment_name, yi_investment, category) not in selected_investments:
                            selected_investments.append((investment_name, yi_investment, category))
                        used_investments.add(investment_name)

                else:
                    # Nếu không thuộc điều kiện, thêm danh mục gốc
                    if (investment_name, yi_investment, category) not in selected_investments:
                        selected_investments.append((investment_name, yi_investment, category))
                    used_investments.add(investment_name)

        # Sắp xếp danh sách selected_investments theo trọng số giảm dần
        selected_investments.sort(key=lambda x: x[1], reverse=True)

        # Tạo danh sách để chứa dữ liệu bảng
        table_data = []

        # Thêm vào dữ liệu bảng
        for idx, (name, weight, category) in enumerate(selected_investments):
            table_data.append([
                f"{idx + 1}",
                name,
                f"{weight:2.2f}%",
                self.criteria_estimated[idx],
                self.risk_estimated[idx]
            ])

        # Hiển thị bảng
        print("\nDanh sách danh mục tối ưu nhất:")
        print(tabulate(table_data, headers=["STT","Danh mục đầu tư", "Trọng số (%)", "Lợi nhuận ước tính", "Rủi ro ước tính"], tablefmt="fancy_grid"))

# Tham số cho thuật toán
pop_size = 5000
bounds = [-5, 5]
max_iter = 1000
switch_iter = max_iter/2

tc1 = random.randint(1, 20) # Lợi nhuận (Positive)
tc2 = random.randint(1, 15) # Rủi ro (negative)
tc3 = random.randint(1, 20) # Thanh khoản (Positive)
tc4 = random.randint(1, 20) # Thời gian đầu tư (average)
tc5 = random.randint(1, 15) # Chi phí (negative)
tc6 = random.randint(1, 20) # Tính ổn định của dòng tiền (Positive)
tc7 = random.randint(1, 20) # Đa dạng hóa (average)

# Dữ liệu => Tổng hợp điểm từ web
#data = [13, 2, 5, 5, 7, 1, 7] # Thay đổi sau mỗi lần vote
data = [tc1, tc2, tc3, tc4, tc5, tc6, tc7]

# Lưu vào file CSV
with open('criteria_data.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Value'])
    for value in data:
        writer.writerow([value])


criteria_manager = CriteriaManager(
    data_file="criteria_data.csv",
    criteria_file="criteria_values.csv",
    file_path="criteria_and_risk_investment.csv"
)

weights = criteria_manager.compute_weights()
criteria_manager.load_criteria_values()
criteria_manager.load_investment_data()

parameters = AlgorithmParameters(
    positive_indices=[0, 2, 5],
    negative_indices=[1, 4],
    diversification_index=6,
    investment_time_index=3
)
# Xác định số biến n_bien dựa trên weights
sorted_indices = np.argsort(weights)[-3:]
n_bien = 10 if 6 in sorted_indices else 6

individual_generator = IndividualGenerator(criteria_manager=criteria_manager, pop_size=pop_size)

population = individual_generator.generate_population()

fitness_function_investment = Fitness_Function_Investment(criteria_manager, parameters)


fitness_investment = np.abs(np.round(np.array(
    [fitness_function_investment.fitness_function(i)
     for i in range(criteria_manager.criteria_values.shape[0])],
    dtype=float), 6))

# Tính fitness cho từng cá thể trong quần thể
fitness = np.abs(Caculate_Fitness_Individual_Generator.calculate_fitness(fitness_investment, population))

# Khởi tạo đối tượng HybridCOA_MVO
hybrid_coa_mvo = HybridCOA_MVO(population, fitness, bounds, max_iter, switch_iter, fitness_investment)

# Gọi phương thức run() để lấy kết quả
best_fitness_investment, best_solution_investment = hybrid_coa_mvo.run()

# Chạy mã với các đối tượng đã được khởi tạo
criteria_manager = CriteriaManager(data_file="criteria_data.csv", criteria_file="criteria_values.csv",
                                   file_path="criteria_and_risk_investment.csv")

# Kiểm tra sự tính toán của các trọng số
weights1 = criteria_manager.compute_weights()

# Tạo ma trận so sánh và hiển thị kết quả
comparison_matrix = criteria_manager.create_comparison_matrix(criteria_manager.normalized_scores)

# Tải dữ liệu
criteria_manager.load_criteria_values()
criteria_manager.load_investment_data()

# Tạo đối tượng DisplayInvestmentsByCategory
display = DisplayInvestmentsByCategory(
    replacement_map="replacement_map.csv",
    categories="categories.csv",
    criteria_manager=criteria_manager
)

# Phân loại và hiển thị
display_ahp_results(comparison_matrix, weights1)
classified_investments = display.classify_investments(best_solution_investment)
display.display_investments_by_category(classified_investments)



